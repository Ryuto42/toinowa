...
> 2026-09-22追記：改善過程を含む詳しい版として維持しています。最新の操作確認・テスト件数・提出前の注意は [サービスレビュー](service-review-2026-09-22.md)、短い公開用の別案は [v3](qiita-article-v3.md) を参照してください。
...
相談への返答はアプリ側の固定文です。学習モデルの「問い返す」「最後にまとめる」という指示に相談を戻さず、理由の説明を強要しないようにしました。最初の修正では「先生の確認一覧に記録しました」と返していましたが、追加の操作確認で、内部処理の説明が相談への応答を妨げると分かりました。現在は通常の返答から記録・連絡に関する文を外し、受け止めと安全の確認を優先しています。先生側の要フォローは維持し、保存失敗は運用ログへ残します。共有範囲を聞かれた場合は、先生や管理者が閲覧できる仕組みであることを説明し、秘密や即時対応は約束しません。先生や保護者へのメール送信を行う機能ではありません。

相談中は短い相づちで学習へ戻らず、本人が再開を希望するまで休止します。最初は「勉強に戻る」との定型一致で判定していましたが、「ちょっと勉強に戻ろうかな」を受け付けられない操作上の問題が見つかりました。さらに「また勉強しよっか！」でも再開できない例があったため、表現一致による許可・拒否を廃止しました。現在は、学習を休止している状態を既存の分類AIに渡し、本人の現在の再開意思を必須の真偽値で判断します。再開時の根拠引用を文字列一致させる条件も外しています。新しい危険の訴えや判定失敗は引き続き支援を優先します。通常の1送信あたりのAI呼び出し回数は増やしていません。案内も「また勉強を再開したくなったら、あなたの言葉で教えてください」という趣旨に調整しました。会話を開き直しても状態を保持し、相談とその応答は新しい学力回答・学習ターン数・評価用の会話・学習の要約から外します。既に保存されている旧版の評価を遡って書き換える処理は行っていません。

再開判定は今回、実際のFlash-Liteにも架空の会話を渡して確認しました。以下の4表現は、いずれもモデルの意味判定で再開になりました。
...
同じ文脈で「はい」「勉強は明日にして今日は休みたい」は再開しませんでした。ただし、この2例はモデル出力がアプリの検証を通らず縮退したため、**意味判定が正しかった例には数えません**。その時点で試験を止め、残りの予定ケースは未実施です。6表現各1回と先行確認1回の計7回で、OrcaRouterの応答由来の確認費用は合計 **$0.000762**。少数の同じ架空文脈での確認であり、あらゆる言い回しへの保証ではありません。生徒の保存済み会話・課題は書き換えず、AI実行記録のみ追加しました。実APIを使わない関連テスト46件と本番ビルドも再確認しました。
...
さらに「再開しようかな」という控えめな意思表示を確認すると、同じモデルでも過去のつらい発言を重視して拒む例がありました。単語を追加するだけでは解決しないため、過去の会話を参照データ、最新発言を判定対象として分け、相談の継続状態はアプリ側で保持しました。AIには「本人の今の意思か」「引用・将来の予定か」「新たな危険があるか」を判断させ、判定の揺れを抑えるためtemperatureを0にしています。

最終版のFlash-Liteでは、次の10例を各1回確認しました。これは調整に用いた架空の文脈と追加の言い換えによる開発確認であり、独立したテストセットの正解率ではありません。

| 最新の発言 | 最終版の実モデルの判断 |
| --- | --- |
| 再開しようかな／また勉強しよっか！ | 再開 |
| そろそろ続きを話してもいい？／気分を変えたいし、さっきの問題もう一度やってみる | 再開 |
| 「続きをやろう」と私は思う | 本人の意思として再開 |
| はい／今日は休んで明日やる／今はまだその気になれない | 休止を維持 |
| 先生が「再開しようかな」と言った | 他人の発言の報告として休止を維持 |
| 再開したいけど今も殴られていて怖い | 新たな危険への支援を優先 |

この追加調整は失敗した案も含め24回、応答で確認できた費用は **$0.002494**（先の7回・$0.000762とは別）でした。最終10例はすべて想定した再開可否になりましたが、意味判定の完全性を保証するものではありません。再開意思のために別のモデル呼び出しを足してはいません。生徒の保存済み対話は変更せず、テスト314件・本番ビルド・変更ファイルのESLintを確認しました。架空入力・実測結果・確認範囲は `docs/data/care-resume-voice-verification-2026-09-22.json` に保存しています。

音声入力でも、認識後に直前のAIの質問全文が生徒の回答へ混ざる報告がありました。調べると、質問全文を認識用の参考情報として渡し、発話後の無音も書き起こしへ送る構造でした。元の録音がないため発生原因を断定はできませんが、質問全文を短いお題名に置き換え、参考データをシステム指示から分離し、無音だけの区切りを送らないよう修正しました。

また、録音を止めると即送信する動作を、認識結果を入力欄で確認・修正してから送信する形に変えました。生徒が実際には説明していない内容を、そのまま評価するのを防ぐためです。最終の認識待ち中は再録音を防ぎ、一部の認識が失敗しても後続の文字を表示し、画面を離れた後の遅延結果を捨てます。音声/APIを模擬したChrome操作と自動テストで確認しましたが、実マイクでの騒音・小声・聞き取り精度は別の検証が必要です。
...
<details>
<summary>相談対応を最初に追加した時点の検証条件と対象コード（返答文の再調整前）</summary>
