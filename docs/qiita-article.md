...
AI HACK 2026のテーマ「業務を自律化するAIエージェント」に向けて開発しました。想定するのは、塾・個別指導などで生徒を継続して担当する先生です。**生徒ごとに課題を考え、説明を読み、次回案を作る仕事をAIへ渡し、先生は案の確認・配信と必要な支援を担います。**

https://toinowa.vercel.app

https://github.com/Ryuto42/toinowa
...
そこで、**自分の言葉で説明してもらう**ことを、理解を捉える手がかりにします。正誤だけでなく、理由や概念のつながりをどう説明するかも観察します。
...
## 2. 先生が教え、生徒が説明し、AIが復習を支える

トイノワでは、**生徒がAIに教える側**に回ります。この対話を、生徒別の課題準備・根拠つきの分析・次回案までつなぎ、先生へ返す業務の流れを作りました。役割は次の4者に分かれます。
...
今回の条件では、GPT-4o miniは必要項目358件が欠落した一方、**JSON検証は9/9通過**しました。形式が正しいことと、必要な情報が揃うことは別でした。
...
補足のCスキャンを含め、Flashは3入力で**150/150項目一致**でした。ただしCは表の見出しを確認できず、人の確認待ちになりました。**数値が読めたことと、自動反映してよいことを分けています。** 同じ資料での改善検証であり、初回9入力の成績を置き換える結果ではありません。[条件・採用判断・復旧の記録](https://github.com/Ryuto42/toinowa/blob/main/docs/exam-reading-reliability.md)
...
```mermaid
flowchart TB
    IN["文字入力<br/>生徒の発言 / 参照テキスト<br/>音声は書き起こし後"]
...
実行条件は `google/gemini-2.5-flash`（`exam`段）、出力上限10,000トークン、制限時間60秒。プロンプト・スキーマ・画像のハッシュと応答を保存しています。画像を直接送る試験で、認証・アップロード・DB処理は対象外です。**有限の攻撃例での結果であり、全攻撃への安全性の保証ではありません。** [実測サマリー](https://github.com/Ryuto42/toinowa/blob/main/docs/exam-injection-live-summary.md)
...
テナント分離の守り方は、経路によって違います。**ユーザーのJWTで動く経路はRLSで守り、service-roleで動く経路はアプリのコードで境界を書いています。** service-role権限の秘密キーは`server-only`のモジュールで扱い、ブラウザ用の公開環境変数には含めません。
...
flowchart TB
    ...
    U --> RA & RB
    RA -->|権限に応じた行だけを許可| RLS --> PG
    RB --> APPCHECK -->|RLSを迂回するためアプリで検査| PG
...
同じ手順で毎回確かめられるようにしています。ゴールデンテストを含む **46ファイル・334件**が通る状態を維持していて、権限とDB処理は実際のDBに繋いで検証します（`npm run test:db`）。
...
先生の準備時間の削減と実生徒の学習効果は、まだ未測定です。次は課題準備にかかる時間、AI案を先生が修正した割合、日を置いて再び説明できるかを測り、業務と学習の両面で確かめます。
