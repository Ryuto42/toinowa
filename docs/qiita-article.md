...
| | 日本の小学校 | 小学校の参加国平均 | 日本の中学校 | 中学校の参加国平均 |
| --- | ---: | ---: | ---: | ---: |
...
この実測を受けて変更したのは、**授業資料を読む `vision` 経路**のフォールバック連鎖です。

```
vision（変更前）: Flash → GPT-4o mini
vision（変更後）: Flash → Flash-Lite（72.2%） → GPT-4o mini（最後段）
```

模試の `exam` 経路は別で、`AI_EXAM_MODEL` の単独モデルです。切り替え先へ黙って降りず、失敗したページだけを再試行して確認待ちに返します。6.9%のモデルへ模試を自動で渡すより安全だからです。
...
実行条件は `google/gemini-2.5-flash`（`exam`段）、出力上限10,000トークン、制限時間60秒。プロンプトとスキーマのSHA-256、画像ごとのハッシュ、応答JSONを実行記録として保存しています。**有限の攻撃例を通っただけで、あらゆる攻撃への安全性を保証するものではありません。**
