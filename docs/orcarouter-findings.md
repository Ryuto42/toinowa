...
`scripts/spike-orcarouter.sh` 等による初期の観測・判断の記録。以下のモデル、価格、残高、採用案は当時の条件に限る。

**現行方針との違い：** 費用未取得を観測済みの0ドルとして扱わず、現在は推定可能ならカタログ価格で見積もり、不明試行も別に記録する。モデル連鎖は `selection.ts` / `routers.ts`、模試専用経路は `exam` を参照する。RAGの未使用実装は削除済み。以下の初期案や単一例から、現在の全体費用・教育品質・一般的なモデル順位を断定しない。現行の根拠は [evidence.md](evidence.md)。
