...
2026-09-22更新。記事・展示・発表で使う共通の根拠集。最新の全体確認は `29bfa61` と今回の未コミット修正が対象。[短い記事案v3](qiita-article-v3.md)・[検証記録](data/service-review-verification-2026-09-22.json)を参照。公開版と同一とは限らない。実装・固定応答の試験・実モデル測定を分けてまとめる。
...
固定応答の判定テストと7画像のdry-runは確認済み。ただし、**この現行条件での実モデルの防御率は未取得**。実行プロトコルが改善されたことと、実画像への防御を実証したことを区別する。[判定試験](../tests/security/exam-injection-protocol.test.ts)。
...
ローカルの先生・生徒・管理者でログインし、テスト生徒への授業メモ→下書き→承認配信→5往復の説明→最終評価→次回案を確認した。承認前と次回の下書きは生徒に見せず、確認待ちの評価は詳細所見を生徒へ返さない。既存の対話はリセットしていない。[画面](assets/service-review/)。
