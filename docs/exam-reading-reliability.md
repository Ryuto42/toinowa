...
**模試解析はGemini 2.5 Flashを採用し、上位の2.5 Proへの変更は見送った。** 上位モデルを実測したうえでの限定的な判断であり、モデル全般の優劣やFlashの最適性を証明したものではない。

この記録が現在の実装・判断。[初回比較](exam-model-benchmark.md)と[構造化抽出v2](exam-model-benchmark-improvement.md)は当時の記録として残す。
...
ブラウザ試験の画面例はデスクトップ・モバイルの両方を取得している。どちらも架空の数値だけを使ったUI検証画面で、ベンチマークの原資料ではない。
