...
const results=path.join(root,'benchmark-data/exams/runs/responses');await fs.mkdir(results,{recursive:true});
...
const cli=path.join(root,'benchmark-data/orcareplay/node_modules/orcareplay/dist/cli.js');const work=path.join(root,'benchmark-data/exams/work');
async function run(args,file){return await new Promise((resolve,reject)=>{const p=spawn(process.execPath,[cli,...args],{cwd:work,env:childEnv,stdio:['ignore','pipe','pipe']});let out='',err='';p.stdout.on('data',d=>out+=d);p.stderr.on('data',d=>{err+=d;});p.on('error',reject);p.on('close',async code=>{const safe=(s)=>s.split(env.BENCHMARK_ORCAROUTER_API_KEY).join('[redacted]');await fs.writeFile(path.join(root,'benchmark-data/exams/runs/'+file+'.json'),safe(out));await fs.writeFile(path.join(root,'benchmark-data/exams/runs/'+file+'.log'),safe(err));console.log(safe(err.slice(-3000)));console.log('exit',code);resolve({code,out});});});}
const mode=process.argv[2]??'record';const id=process.argv[3]??'A_scan';
if(mode==='record'){
 const req=path.join(root,'benchmark-data/exams/requests/'+id+'.json');
 ...
 await run(['compare',process.argv[4]??'last','--from','1','--models','google/gemini-2.5-flash,openai/gpt-4o-mini','--no-fs','--upstream-openai','https://api.orcarouter.ai','--json'],id+'-compare');
