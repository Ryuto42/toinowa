...
for(const id of cases){
 await run(['record',id]);const meta=JSON.parse(await fs.readFile(`benchmark-data/exams/runs${suffix}/${id}-record.json`,'utf8'));
 if(meta.exitCode!==0||meta.modelExchanges!==1)throw Error('Invalid seed '+id+'; stop for inspection');
 await run(['compare',id,meta.runId]);
 const compared=JSON.parse(await fs.readFile(`benchmark-data/exams/runs${suffix}/${id}-compare.json`,'utf8'));
