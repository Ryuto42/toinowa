...
const {body}=JSON.parse(await fs.readFile('benchmark-data/exams/requests/A_scan.json','utf8'));const records=[];
for(const model of [body.model,'google/gemini-2.5-flash','openai/gpt-4o-mini']){
...
await fs.writeFile('benchmark-data/exams/runs/replay-payload-check.json',JSON.stringify({orcareplay:'0.4.0',method:'Actual proxy with offline upstream stub; no external API calls',records},null,2));console.log(records);
