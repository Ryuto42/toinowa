...
export async function topicStudentContext(context: AuthContext, studentId: string, classroomId: string, database?: SupabaseClient<Database>) {
  const db = database ?? await createClient();
