...
export function asksToResume(text: string): boolean {
  return /^(勉強に戻る|学習に戻る|課題に戻る|勉強を再開します|学習を再開します)[。！!\s]*$/u.test(text.trim());
}
...
export function immediateCare(input: string): CareDecision | null {
  ...
  if (/今.{0,8}(殴られて|襲われて|血が止まらない)|今から.{0,8}(飛び降り|自分を傷つけ)/u.test(text)) {
    return { category: 'danger', evidence: text.slice(0, 240), reason: '差し迫った危険を示す発言。至急の確認が必要です。' };
  }
  const signal = detectWellbeing(text);
  if (signal) return { category: signal.category, evidence: text.slice(0, 240), reason: '明確な安全・相談の合図。事実関係は先生の確認が必要です。' };
  if (/(学校|家|教室).{0,12}(行きたくない|帰りたくない|こわい|怖い)|助けて|たすけて/u.test(text)
    && !/(問題|宿題|解き方|計算|数式|数学|英語).{0,10}(助けて|たすけて)/u.test(text)) {
    return { category: 'distress', evidence: text.slice(0, 240), reason: '助けを求める発言を学習より優先しました。' };
  }
  return null;
}
...
export function fallbackCare(text: string): CareDecision {
  ...
  if (/^(ば[ー〜～]*か[、。!！\s]*|バ[ー〜～]*カ[、。!！\s]*|馬鹿[、。!！\s]*)+$/u.test(text.trim())) {
    return { category: 'hostility', evidence: text.slice(0, 240), reason: '相手を傷つける言葉への境界を伝えます。' };
  }
  return { category: 'unavailable', evidence: '', reason: '意味の判定を完了できませんでした。' };
}
...
export function careReply(category: CareDecision['category'], recorded: boolean, continued = false): string {
  ...
  const receipt = recorded ? 'この相談は担当の先生が確認する一覧に記録しました。すぐに確認されるとは限りません。' : '先生の確認一覧への記録を完了できませんでした。信頼できる大人に直接伝えてください。';
  if (category === 'self_harm' || category === 'danger') {
    return `話してくれてありがとう。今は勉強より、あなたの安全を大切にしたいです。ひとりで抱えず、近くの信頼できる大人に今すぐ助けを求めてください。今すぐ危険がある、けがをしている場合は110や119に連絡してください。${receipt}`;
  }
  if (continued) return `話してくれてありがとう。無理に詳しく話したり、勉強を続けたりしなくて大丈夫です。安心して話せる大人に相談してください。${receipt} 学習を再開したいときだけ「勉強に戻る」と送れます。`;
  return `話してくれてありがとう。つらいときは、今は勉強を休んで大丈夫です。${receipt} 今、安心できる場所にいますか？ 近くの信頼できる大人にも相談してください。`;
}
