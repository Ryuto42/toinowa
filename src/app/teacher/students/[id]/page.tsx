...
export default async function TeacherStudentPage({ params }: PageProps<'/teacher/students/[id]'>) {
  ...
  const scores = [...latest.values()].flatMap((row) => row.score === null ? [] : [Number(row.override_score ?? row.score)]);
...
<div className="mb-5 rounded-xl bg-emerald-50 p-4 text-sm leading-7"><p><strong>目標：</strong>{profile.data?.learning_goal || '未設定'}</p><p><strong>苦手な範囲：</strong>{profile.data?.weak_areas || '未設定'}</p><details><summary className="cursor-pointer">登録時の模試結果</summary><p className="whitespace-pre-wrap">{profile.data?.exam_results || '未登録'}</p></details></div>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-2">
      <Panel title="概念説明とAI評価" description="会話全体から、先生が確認しやすい根拠と観点別の分析を表示します。">
        {latest.size ? <div className="space-y-6">{[...latest.values()].map((item) => {
          ...
          return <article key={item.id} className="rounded-xl border border-slate-200 p-4">
            <div className="flex items-center justify-between gap-3"><p className="font-bold">{item.concepts?.name ?? '概念'}</p><StatusPill tone={item.reviewer_status === 'pending_review' ? 'amber' : 'emerald'}>{item.reviewer_status === 'pending_review' ? '確認が必要' : '自動分析済み'}</StatusPill></div>
            <div className="mt-3"><ScoreBar value={item.override_score ?? item.score} /></div>
            <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-slate-500">AIの確信度</dt><dd className="font-bold">{Math.round(Number(item.confidence) * 100)}%</dd></div><div><dt className="text-slate-500">評価日時</dt><dd className="font-bold">{formatDateTime(item.created_at)}</dd></div></dl>
            <div className="mt-5 rounded-xl bg-[#f7faf9] p-4"><p className="font-bold text-[#17233d]">AIからの総合所見</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-700">{item.difficulty_reason ?? '記録なし'}</p></div>
            {Object.keys(dimensions).length ? <div className="mt-5"><p className="font-bold">観点別の理解度</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{Object.entries(dimensionLabels).map(([key, label]) => <div key={key}><div className="mb-1 flex justify-between text-xs"><span>{label}</span><span className="font-bold">{Math.round(Number(dimensions[key] ?? 0) * 100)}%</span></div><ScoreBar value={typeof dimensions[key] === 'number' ? dimensions[key] as number : Number(dimensions[key] ?? 0)} /></div>)}</div></div> : null}
