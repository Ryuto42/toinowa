...
export default async function StudentRecordsPage() {
  ...
  return <div><PageTitle title="学習の振り返り" description="最後まで取り組んだ課題を、一つずつ振り返ろう。" /><div className="grid gap-5 lg:grid-cols-2">{feedback.map(item => <FeedbackCard key={item.id} feedback={item} />)}</div>{!feedback.length ? <EmptyState>対話が終わり、分析ができると振り返りが表示されます。</EmptyState> : null}</div>;
}
