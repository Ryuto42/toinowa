...
export default async function EditUserPage({ params }: PageProps<'/admin/users/[id]'>) {
  ...
  return <div><PageTitle title="ユーザー情報を編集" description={`${user.data.display_name}さんの登録情報を更新します。`} /><Panel title={user.data.role === 'student' ? '基本情報・学習プロフィール' : '基本情報'}><UserEditForm user={user.data} profile={profile?.data ?? null} /></Panel><PasswordReset userId={id} name={user.data.display_name} /></div>;
}
