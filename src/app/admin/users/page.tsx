...
import { ExamAnalysisList } from '@/components/admin/exam-analysis-list';
...
export default async function UsersPage() {
  ...
  return <div><PageTitle title="ユーザー管理" description="生徒・先生・管理者の追加と、アクセス状態の変更を行います。" action={<UserForm classrooms={classrooms.data ?? []} teachers={(data ?? []).filter(user => user.role !== 'student' && user.status === 'active').map(user => ({ id: user.id, name: user.display_name }))} />} /><Panel><UserList key={(data ?? []).map(user => `${user.id}:${user.status}:${user.archived_at}`).join(',')} initial={data ?? []} /></Panel><ExamAnalysisList /></div>;
}
