...
export default async function AdminOverviewPage() {
  ...
  return <div>
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <MetricCard label="要フォロー" value={escalations.count ?? 0} tone={escalations.count ? 'rose' : 'slate'} />
    </div>
    <div className="mt-6 grid gap-6 xl:grid-cols-2">
      <Panel title="学習状況" description="生徒のAIワークと評価の蓄積です。">
        ...
        <div className="mt-5 flex flex-wrap gap-3"><Link href="/teacher/students" className="rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white">学習フィードバックを見る</Link><Link href="/admin/interventions" className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-bold text-slate-700">要フォローを見る</Link></div>
      </Panel>
