...
import { ClassroomManager, type ClassroomRow, type MemberOption } from '@/components/admin/classroom-manager';

export default async function AdminClassroomsPage() {
  const context = await requireRole('admin');
  const db = await createClient();
  const [classrooms, users, enrollments] = await Promise.all([
    db.from('classrooms').select('id,name,subject,grade').eq('tenant_id', context.tenantId).order('name'),
  ...
  const orphanTeachers = members.filter((m) => m.role === 'teacher'
    && !rows.some((row) => row.teacherIds.includes(m.id)));

  return <div>
    <PageTitle title="クラス管理" />
    <p className="-mt-6 mb-8 text-sm text-[#60708d]">
      クラスを作り、担当する先生と在籍する生徒を紐付けます。
      担当クラスが無い先生は、生徒もお題も見ることができません。
    </p>
    {orphanTeachers.length ? <div className="mb-6 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm">
      <p className="font-bold text-amber-900">担当クラスが無い先生が{orphanTeachers.length}人います</p>
      <p className="mt-1 text-amber-800">
        {orphanTeachers.map((t) => t.name).join('、')} — このままでは何も表示されません。クラスに紐付けてください。
      </p>
    </div> : null}
    <Panel title="クラス">
