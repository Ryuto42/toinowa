...
import { tutorialOpening } from '@/lib/tutorial/content';
import { studentTutorialAudience } from '@/lib/tutorial/student';
export async function POST() {
  try {
    const context = await requireRole('student');
    const audience = await studentTutorialAudience(context.tenantId, context.userId);
    const result = await adminDb().rpc('start_student_tutorial', { p_tenant: context.tenantId, p_student: context.userId, p_opening: tutorialOpening(audience) });
