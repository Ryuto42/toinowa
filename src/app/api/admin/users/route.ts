...
}).refine(value => value.role !== 'student' || Boolean(value.intake && value.loginIdentifier), '生徒のログインID・学年・目的・クラスを入力してください').refine(value => value.role === 'student' || Boolean(value.email && value.password), '先生・管理者のメールアドレスとパスワードを入力してください');
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const db = adminDb();
    const credentials = body.role === 'student' ? generateStudentCredentials() : null;
    ...
    const codes = credentials ? await (await createClient()).from('school_codes').select('code').eq('tenant_id', context.tenantId).eq('active', true).order('code').limit(1) : null;
    if (codes?.error) throw new Error(codes.error.message);
    if (credentials && !codes?.data?.length) throw new ApiInputError('有効な所属コードを設定してから生徒を登録してください');
    if (body.intake) {
    ...
    const { data: created, error: authError } = await db.auth.admin.createUser({ email: credentials?.authEmail ?? body.email!, password: credentials?.initialPassword ?? body.password!, email_confirm: true, user_metadata: { tenant_id: context.tenantId, login_identifier: loginIdentifier } });
    if (authError || !created.user) throw new Error(authError?.message ?? 'auth user create failed');
    authUserId = created.user.id;
    const { error: userError } = await db.from('users').insert({ id: authUserId, tenant_id: context.tenantId, role: body.role, display_name: body.displayName, email: credentials ? null : body.email!, login_identifier: loginIdentifier, must_change_password: Boolean(credentials), status: 'active' });
    ...
    return json({ user: { id: authUserId, role: body.role, displayName: body.displayName }, ...(credentials ? { credentials: { organizationCode: codes!.data![0].code, loginIdentifier, initialPassword: credentials.initialPassword } } : {}) }, { status: 201 });
  } catch (error) {
