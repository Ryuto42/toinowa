...
export function SettingsForm({ initial }: { initial: { dailyTimeLimitMin: number; learningPreferences: unknown; notificationSettings: unknown; consentStatus: unknown } }) {
  ...
  return <form onSubmit={save} className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6">
    <label className="block"><span className="text-sm font-bold">1日の目標学習時間</span><input type="number" min={5} max={240} value={minutes} onChange={(e) => setMinutes(Number(e.target.value))} className="mt-2 block w-full rounded-xl border border-slate-300 px-3 py-2"/><span className="mt-1 block text-xs text-slate-500">5〜240分の範囲で設定できます。</span></label>
