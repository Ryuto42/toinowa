...
export function Panel({ title, description, children, action }: { title: string; description?: string; children: React.ReactNode; action?: React.ReactNode }) {
  return <section className="rounded-[22px] border border-[#e3eaee] bg-white"><header className="flex items-start justify-between border-b border-[#eef2f3] px-7 py-5"><div><h2 className="font-bold text-[#17233d]">{title}</h2>{description ? <p className="mt-1 text-xs text-[#8a9ab2]">{description}</p> : null}</div>{action}</header><div className="p-7">{children}</div></section>;
}
